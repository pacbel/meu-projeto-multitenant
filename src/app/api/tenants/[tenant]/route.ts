...
export async function GET(request: NextRequest, { params }: { params: { tenant: string } }): Promise<Response> {
  try {
    const { tenant } = params;
    const isAllowed = await TenantService.isTenantAllowed(tenant);

    return Response.json({ tenant, isAllowed }, { status: 200 });
  } catch (error) {
...
export async function DELETE(request: NextRequest, { params }: { params: { tenant: string } }): Promise<Response> {
  try {
    const { tenant } = params;
